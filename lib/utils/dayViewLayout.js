...
function getStyledEvents(_ref7) {
  ...
  var idx = 0;
  var prevNbrOfColumns = 0;
  var prevParentHasChildren = false;
  ...
  var _loop = function _loop() {
    ...
    if (prevParentHasChildren) {
      var hasOverlapWithPrevParentChild = events.some(function (_event, eventIdx) {
        if (eventIdx >= idx) return false;
        return isChild(eventIdx, idx, { events: events, startAccessor: startAccessor, endAccessor: endAccessor, min: min, totalMin: totalMin });
      });

...
function getStyledAvailabilities(_ref8) {
  ...
  var availabilities = unsortedAvailabilities.sort(function (a, b) {
  ...
  for (var availabilityIndex = 0; availabilityIndex < availabilities.length; availabilityIndex++) {
    var availability = availabilities[availabilityIndex];
    var grouped = false;

    // Check for overlap with existing groups
    var _overlapKeys = Object.keys(overlapMap);
    for (var groupIndex = 0; groupIndex < _overlapKeys.length; groupIndex++) {
      var groupKey = _overlapKeys[groupIndex];
      var group = overlapMap[groupKey];
  ...
  var overlapKeys = Object.keys(overlapMap);
  for (var _groupIndex = 0; _groupIndex < overlapKeys.length; _groupIndex++) {
    var _columnIndex = overlapKeys[_groupIndex];
    var _group = overlapMap[_columnIndex];
    for (var _availabilityIndex = 0; _availabilityIndex < _group.length; _availabilityIndex++) {
      var _availability = _group[_availabilityIndex];

      var _getYStyles3 = getYStyles(availabilities.indexOf(_availability), helperArgs),
          height = _getYStyles3.height,
          top = _getYStyles3.top;

      var isMultiColumn = Object.keys(overlapMap).length > 1;
      var xOffset = isMultiColumn ? _groupIndex * 25 : undefined;

...
exports.makeEventOrAvailabilityFilter = makeEventOrAvailabilityFilter;
...
function makeEventOrAvailabilityFilter(date, startAccessor, endAccessor) {
  return function filterDayEventsOrAvailabilities(eventOrAvailability) {
    var start = (0, _accessors.accessor)(eventOrAvailability, startAccessor);
    return _dates2.default.inRange(date, start, (0, _accessors.accessor)(eventOrAvailability, endAccessor), 'day') && _dates2.default.eq(date, start, 'day'); // ignore back end of long range events for now
  };
}